     ...
     {"encode map",
      fun() ->
              {ok, Map} = ?MAP_TYPE:update({update,
                                            [
                                             {update, {<<"a">>, riak_dt_orswot},{add_all, [<<"a">>, <<"b">>, <<"c">>]}},
    ...
    [
     ...
     {"decode map ops",
      fun() ->
              ...
              ?assertEqual({map, {update, [{remove, {<<"a">>, ?EMCNTR_TYPE}}, {remove, {<<"a">>, riak_dt_orswot}}]}, undefined},
              ...
              {struct, [_Type, _Value, {<<"context">>, JSONCtx}]} = fetch_response_to_json(set, ?MAP_TYPE:value(Map),
                                                                                           BinContext,
                                                                                           ModMap),
              ?assertMatch({map, {update, [{remove, {<<"a">>, riak_dt_orswot}}]}, BinContext},
