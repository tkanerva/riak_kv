...
do_update(node_writeonce_put) ->
    exometer:update([?PFX, ?APP, node, puts, writeonce_put], 1);
do_update(vnode_writeonce_put) ->
    exometer:update([?PFX, ?APP, vnode, puts, writeonce_put], 1).
...
stats() ->
    ...
    [{[vnode, gets], spiral, [], [{one  , vnode_gets},
     ...
     {[node, puts, writeonce_put], counter, [], [{value,node_writeonce_put_total}]},
     {[vnode, puts, writeonce_put], counter, [], [{value,vnode_writeonce_put_total}]},
