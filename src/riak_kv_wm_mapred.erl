...
pipe_mapred_nonchunked(RD, State, Mrc) ->
    case riak_kv_mrc_pipe:collect_sink(Mrc) of
        {ok, Results} ->
            riak_kv_wm_utils:log_http_access(success, RD, riak_core_security:get_username(State#state.security)),
            JSONResults =
        ...
        {error, {sink_died, Error}} ->
            ...
            {{halt, 500}, send_error(Error, RD), State};
        {error, timeout} ->
            riak_kv_wm_utils:log_http_access(failure, RD, riak_core_security:get_username(State#state.security), timeout),
